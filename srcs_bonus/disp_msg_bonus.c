...
/*   Updated: 2022/05/17 18:04:16 by blevrel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	disp_moves_in_window(t_all *struc)
{
	...
	str = "Moves : ";
	moves = ft_itoa(struc->player.count++);
	dest = ft_strjoin(str, moves);
	mlx_string_put(struc->mlx, struc->window.win_ptr, 10, 10, 0x00FFFFFF, dest);
	free(moves);
	free(dest);
}
